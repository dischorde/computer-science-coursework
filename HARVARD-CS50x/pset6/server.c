...
char* indexes(const char* path)
{
    char iphp[11] = "index.php";
    char ihtml[12] = "index.html";
    char* pathip = malloc(strlen(root) + strlen(path) + strlen(iphp));
    char* pathih = malloc(strlen(root) + strlen(path) + strlen(ihtml));
    
    // concatenate indexes to paths
    strcpy(pathip, path);
    strcpy(pathih, path);
    strcat(pathip, iphp);
    strcat(pathih, ihtml);
        
    // check to see if index.php exists
    if (access(pathip, R_OK) == 0)
    {
        free(pathih);
        return pathip;
    }    
    
    if (access(pathih, R_OK) == 0)
    {
        free(pathip);
        return pathih;
    }
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    // prepare a buffer, storage space, and initialize variables
    BYTE buffer[BYTES];
    BYTE* storage = NULL;
    *length = 0;
    size_t numread = 0;
    ...
    do
    {   // read BYTES bytes into buffer until EOF (numread may be < BYTES but > 0) 
        numread = fread(&buffer, 1, BYTES, file);
        
        // resize the storage by BYTES
        storage = realloc(storage, *length + BYTES);
        if (storage == NULL)
        {
            return false;
        }
        // copy the buffer to the spot opened up, using length
        memcpy(storage + *length, buffer, BYTES);
        
        // update length by amount of valid bytes in buffer
        *length += numread;
    }
    while (numread > 0);
    
    // if length == 0, something went wrong
    if (*length == 0)
    {
        return false;
    }
    
    // realloc to match length exactly
...
const char* lookup(const char* path)
{
    // find the last instance of ., which starts file extension
    ...
    // return proper MIME type for each supported extension, ignoring case
    if (strcasecmp(extension, ".css") == 0)
    {
        char* type = malloc(9);
        strncpy(type, "text/css", 9);
        return type;
    }
    ...
    {
        char* type = malloc(16);
        strncpy(type, "text/javascript", 16);
        return type;
    }
...
{
    // find pointer to beginning of each section, assuming proper formatting
    char* req_tgt = strchr(line, ' ') + 1;
    char* version = strchr(req_tgt, ' ') + 1;
    
    // ensure proper formatting (if so, the above should have worked), returning false otherwise
    if (line == NULL || req_tgt == NULL || version == NULL || strchr(version, ' ') != NULL)
    {
        error(400);
        return false;
    }
    
    // check that first three characters of method are "GET"
    if (strncmp(line, "GET", 3) != 0 || req_tgt - line - 1 > 3)
    {
        error(405);
        return false;
    }
    
    // check request-target starts with '/'
    if (req_tgt[0] != '/')
    {
        error(501);
        return false;
    ...
    // check request-target does not contain any "
    ...
    if (strchr(req_tgt, '?') != NULL)
    {
        // save the remainder of req_tgt of length qlen to query
        char* qstrt = strchr(req_tgt, '?') + 1;
        int qlen = version - qstrt - 1;
        strncpy(query, qstrt, qlen);
        query[qlen] = '\0';
        
        // save everything before the ? to absolute path
        int abslen = qstrt - req_tgt - 1;
        strncpy(abs_path, req_tgt, abslen);
        abs_path[abslen] = '\0';
        return true;
    }
